...
export default function XpProgressCircle({ level, setLevel }) {
  const xpThresholds = [700, 2100, 4900, 13300, 17500,25400,31500,38500,55300,63700,72100,80500,88900,111300,122500,133700,144900,158900,186900];
  ...
  const currentThreshold = xpThresholds[level - 1] || 14700;
  const progress = Math.min(xp / currentThreshold, 1);
  ...
  const addXp = (amount) => {
    ...
    while (newLevel - 1 < xpThresholds.length && xpLeft >= xpThresholds[newLevel - 1]) {
      xpLeft -= xpThresholds[newLevel - 1];
      newLevel++;
      leveledUp = true;
    }
  ...
  const handleDifficultyXpAdd = () => {
    ...
    addXp(totalXp);
    setXpGainResult(`+${totalXp} XP (base: ${base} × dés: ${diceRolls.join(" + ")} = ${diceTotal})`);
      ...
      {showDifficultySelector && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl w-80">
            <h3 className="font-bold text-lg text-black mb-4">Choisissez la difficulté :</h3>
            <select
